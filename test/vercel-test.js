...
(async () => {
  ...
  console.log('URL: https://math-tug-of-war-nine.vercel.app\n');
  ...
  await p1.goto('https://math-tug-of-war-nine.vercel.app', { waitUntil: 'networkidle0' });
  await delay(2000);
  console.log('P1: Opened deployed URL');
  ...
  await p2.goto('https://math-tug-of-war-nine.vercel.app', { waitUntil: 'networkidle0' });
  await delay(2000);
  console.log('P2: Opened deployed URL');
  ...
  if (!p1InGame || !p2InGame) {
    ...
    await p1.screenshot({ path: path.join(__dirname, 'vercel-debug-p1.png') });
    await p2.screenshot({ path: path.join(__dirname, 'vercel-debug-p2.png') });
    await browser.close();
    process.exit(1);
  }
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    try {
      const prompt = qPrompt.replace(/×/g, '*').replace(/÷/g, '/').replace(/−/g, '-');
      if (prompt.includes('/')) {
  ...
  await p1.screenshot({ path: path.join(__dirname, 'vercel-p1-final.png') });
  await p2.screenshot({ path: path.join(__dirname, 'vercel-p2-final.png') });
  
  await browser.close();
  console.log('\n✅ VERCEL TEST COMPLETE');
})();
